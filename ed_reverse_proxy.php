...
class ed_reverse_proxy
{
    ...
    private $enable_cross_site_scripting = false;
    private $debug = false;
    ...
    public function __construct()
    {
        ...
        add_action('admin_init', array($this, 'cross_site_scripting_customization'));

        if($this->debug){
            add_action('wp_footer',array($this,'debug_rp_request'));
        }

        add_shortcode('rp_link',array($this,'generate_rp_url'));

        add_filter('wp_redirect',array($this,'fix_redirect_urls'));
        add_filter('wpseo_stylesheet_url',array($this,'wpseo_stylesheet_url'));

    }

    function debug_rp_request(){
        echo '<pre>';
        print_r($_SERVER);
        echo '</pre>';
    }

    function wpseo_stylesheet_url(){
            ...
            if (!empty($proxy_domains)) {
                foreach ($proxy_domains as $key => $proxy) {
                    ...
                    if (in_array($_SERVER['REMOTE_HOST'], $ips) || in_array($_SERVER['REMOTE_ADDR'], $ips) || true ) {
                        ...
                        $url_extension = pathinfo($_SERVER['REQUEST_URI']);
                        if( isset($url_extension['extension']) && $url_extension['extension'] == 'php' ){
                            $request_uri = rtrim($_SERVER['REQUEST_URI'], '/');
                        }
    ...
    function cross_site_scripting_customization()
    {
        if ($this->enable_cross_site_scripting && is_ajax()) {
            $proxy_domains = get_option("proxy_domains");

            if (isset($_SERVER['HTTP_REFERER'])) {
                $domain_info = parse_url($_SERVER['HTTP_REFERER']);

                if( isset( $proxy_domains[$domain_info['host']] ) ){
                    $host = $proxy_domains[$domain_info['host']]['domain'];
